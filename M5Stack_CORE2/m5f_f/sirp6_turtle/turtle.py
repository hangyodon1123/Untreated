    ...
    for point in turtle_data:
      ...
      if (0<=xpixel<=383 and 0<=ypixel<=191):
        if not inbuffer(xpixel,ypixel):
          turtle_buffer+=[[xpixel,ypixel,lcd.readPixel(xpixel,ypixel)]]
        lcd.pixel(xpixel,ypixel,c)
...
def erase_turtle():
  ...
  for i in range(1,len(turtle_buffer)):
    ...
    if turtle_buffer[i][2]!=None :
      ...
      lastcolor=lcd.WHITE
    lcd.pixel(xpixel,ypixel,lastcolor)
  turtle_buffer=[[]]

def pen_brush(x,y,turtle_color):
  global frame_count
  erase_turtle()
  xpixel=int(round(x+160))
  ypixel=int(round(-y+120))
  if writing==True and (0<=xpixel<=319 and 0<=ypixel<=239) :
    colorpixel=lcd.BLACK
    lcd.pixel(xpixel,ypixel,colorpixel)
    frame_count+=1
    if turtle_speed!=0:
...
def pencolor(*c):
  global turtle_color
  colornames={"black":lcd.BLACK,"blue":lcd.BLUE,"green":lcd.GREEN,"red":lcd.RED,"cyan":lcd.CYAN,"yellow":lcd.YELLOW,"magenta":lcd.MAGENTA,"white":lcd.WHITE,"orange":lcd.ORANGE,"purple":lcd.PURPLE,"brown":lcd.OLIVE,"pink":lcd.PINK,"grey":lcd.DARKGREY}
...
def write(text):
  ...
  c=lcd.BLACK
  lcd.text(xpixel,ypixel,str(text),c)
  #casioplot.show_screen()
