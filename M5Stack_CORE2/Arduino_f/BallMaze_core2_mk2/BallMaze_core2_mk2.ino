...
#if defined( LGFX_M5STACK_CORE2 ) || defined( LGFX_M5STACK ) || defined( ARDUINO_M5Stack_Core_ESP32 ) || defined( ARDUINO_M5STACK_FIRE ) // M5Stack
...
//----------add
static LGFX_Sprite sp_chr[4](&lcd);
static int sp_chr_i = 0;
//----------add end
...
//----------add
static void chr_make(void)
{
  for (uint32_t i = 0; i < 3; ++i) {
    sp_chr[i].clear(0);
    sp_chr[i].setColorDepth(16);
    sp_chr[i].createSprite(8, 8);
    sp_chr[i].setPivot(4, 4);
    for (uint32_t x = 0; x < 8; ++x) {
      for (uint32_t y = 0; y < 8; ++y) {
        if (chr_data[i][y][x] == 1) {
          sp_chr[i].drawPixel(x, y, 0xFFFF00U);
        }
        else {
          sp_chr[i].drawPixel(x, y, TFT_BLACK);
        }
      }
    }
  }
...
//----------comment out end
//----------add
    sp_chr[sp_chr_i].pushRotateZoom(px, py, angle, 2, 2);
    switch(sp_chr_i)
    {
        case 0:
        case 1:
