...
void loop(void)
{
  ...
  int sec = RTCtime.Seconds;
  int minu = RTCtime.Minutes;
  int hour = RTCtime.Hours;

  static int prev_minu = -1;
...
//----------
  ...
  for (int div_y = 0; div_y < lcd.height(); div_y += sprite_height)
  {
    ...
    float angle = (float)(sec * 6 + count / 6);
    float angle2 = (float)(minu * 6 + angle / 60);
    //----------
//  sprites[flip].clear();
//* // draw background
    ...
    if (div_y == 0)
    {
      ...
      sprites[flip].setCursor(80,0);
      sprites[flip].printf("%02d:%02d:%02d", hour, minu, sec);    
    ...
    for (int i = 0; i < 60; i++)
    {
      float a = fmodf(360.0f + angle - i * 6.0f, 360.0f);
    ...
    for (int i = 0; i < 60; i++)
    {
      float a = fmodf(360.0f + angle2 - i * 6.0f, 360.0f);
    ...
    for (int i = 0; i < 60; i += 5)
    {
      ...
      sp.drawNumber(int(i), sp.width()>>1, sp.height()>>1);
      sp.pushRotateZoomWithAA(&sprites[flip], lcd.width() / 2, y_center - div_y, a, zoom_x, zoom_y, 0);
    }
