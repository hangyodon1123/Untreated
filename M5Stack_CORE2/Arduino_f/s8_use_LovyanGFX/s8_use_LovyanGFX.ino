...
static char *time_d[] = {"○","一","二","三","四","五","六","七","八","九"};
...
//  if (render.loadFont("/ipam.ttf"))
  if (render.loadFont("/BABAmoji.ttf"))
  {
...
//  lcd.fillScreen(TFT_BLACK);
  ...
  for ( int i=0; i<=4; ++i) {
    M5.lcd.fillRect((64*i),24+8,64,64+6+1,TFT_BLUE);
    disp_k(data_s[i]);
  }
  render.printf("\n");
  ...
  render.printf(" ");
  for ( int i=6; i<=7; ++i) {
    M5.lcd.fillRect(60+(120*(i-6)),24+64+16,120,120+12+4,TFT_BLUE);
    disp_k(data_s[i]);
  }
}
